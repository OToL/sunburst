...
sb::MemoryArena sb::reserveVirtualMemory(usize size, [[maybe_unused]] char const * name)
{
    ...
    if (sb_expected(nullptr != mem_ptr, "Failed to reserve {} bytes for client '{}' with error '{}'", size, name,
                 GetLastError()))
    {
        return {mem_ptr, size};
    }
...
bool sb::mapVirtualMemory(MemoryArena arena)
{
    ...
    if (sb_expected(nullptr != VirtualAlloc(arena.data, arena.size, MEM_COMMIT, PAGE_READWRITE),
...
bool sb::unmapVirtualMemory(MemoryArena arena)
{
    ...
    if (sb_expected(VirtualFree(arena.data, arena.size, MEM_DECOMMIT), "Failed to unmap {} bytes with error '{}'",
...
bool sb::releaseVirtualMemory(void * base_mem)
{
    ...
    if (sb_expected(VirtualFree(base_mem, 0, MEM_RELEASE), "Failed to release memory with error '{}'", GetLastError()))
    {
        return true;
    }
