...
TEST_CASE("String to buffer", "[conversion]")
{
    ...
    auto byte_cnt = toString("test", test_buffer);
...
TEST_CASE("Integral to string", "[conversion]")
{
    ...
    // i8
    auto byte_cnt = toString((i8)1, test_buffer);
    REQUIRE(0 == strcmp("1", &test_buffer[0]));
    REQUIRE(byte_cnt == 1U);

    byte_cnt = toString((i8)-1, test_buffer);
    REQUIRE(0 == strcmp("-1", &test_buffer[0]));
    REQUIRE(byte_cnt == 2U);

    // i16
    byte_cnt = toString((i16)256, test_buffer);
    ...
    // i32
    byte_cnt = toString((i32)65536, test_buffer);
    ...
    // i64
    byte_cnt = toString((i64)4294967296, test_buffer);
...
TEST_CASE("Unsigned integral to buffer", "[conversion]")
{
    ...
    // u8
    auto byte_cnt = toString((u8)255, test_buffer);
    REQUIRE(0 == strcmp("255", &test_buffer[0]));
    REQUIRE(byte_cnt == 3U);

    // u16
    byte_cnt = toString((u16)65535, test_buffer);
    REQUIRE(0 == strcmp("65535", &test_buffer[0]));
    REQUIRE(byte_cnt == 5U);

    // u32
    byte_cnt = toString((u32)4294967295U, test_buffer);
    REQUIRE(0 == strcmp("4294967295", &test_buffer[0]));
    REQUIRE(byte_cnt == 10U);

    // u64
    byte_cnt = toString((u64)18446744073709551615ULL, test_buffer);
