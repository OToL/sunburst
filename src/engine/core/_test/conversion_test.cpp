...
TEST(CONVERSION, StringToSpan)
{
    ...
    auto byte_cnt = stringCast("test", test_buffer);
...
TEST(CONVERSION, SignedIntToSpan)
{
    ...
    // si8
    auto byte_cnt = stringCast((si8)1, test_buffer);
    EXPECT_STREQ("1", test_buffer);
    EXPECT_EQ(byte_cnt, 1U);

    byte_cnt = stringCast((si8)-1, test_buffer);
    EXPECT_STREQ("-1", test_buffer);
    EXPECT_EQ(byte_cnt, 2U);

    // si16
    byte_cnt = stringCast((si16)256, test_buffer);
    ...
    // si32
    byte_cnt = stringCast((si32)65536, test_buffer);
    ...
    // si64
    byte_cnt = stringCast((si64)4294967296, test_buffer);
...
TEST(CONVERSION, UnsignedIntToSpan)
{
    ...
    // ui8
    auto byte_cnt = stringCast((ui8)255, test_buffer);
    EXPECT_STREQ("255", test_buffer);
    EXPECT_EQ(byte_cnt, 3U);

    // ui16
    byte_cnt = stringCast((ui16)65535, test_buffer);
    EXPECT_STREQ("65535", test_buffer);
    EXPECT_EQ(byte_cnt, 5U);

    // ui32
    byte_cnt = stringCast((ui32)4294967295U, test_buffer);
    EXPECT_STREQ("4294967295", test_buffer);
    EXPECT_EQ(byte_cnt, 10U);

    // ui64
    byte_cnt = stringCast((ui64)18446744073709551615ULL, test_buffer);
