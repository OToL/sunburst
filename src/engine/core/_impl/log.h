...
#    define sbLogImpl(lvl, file, line, msg, ...) \
        (!sb::detail::gs_log_quiet && ((ui8)lvl) <= ((ui8)sb::detail::gs_log_min_level)) && (sb::detail::LogFilter<sbCTFIsEnabled(LOG_FACILITY)>::logMessage(lvl, file, line, msg, ##__VA_ARGS__), true)
